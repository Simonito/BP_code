...
class ResBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, timestep_emb_dim: int=1280) -> None:
        super().__init__()
...
class TransformerBlock(nn.Module):
    def __init__(self, in_dim: int, num_heads: int, patch_size: int, img_size: int, cross_attn_ctx_dim: Optional[int] = None) -> None:
        super().__init__()
...
class UNETR(nn.Module):
    def __init__(self,
        ...
        self.encoder_layers = nn.ModuleList([
            ...
            nn.ModuleList([
                ResBlock(first_channel, 64, timestep_emb_dim=time_embed_dim),
                ResBlock(64, 64, timestep_emb_dim=time_embed_dim),
            ]),
            ...
            nn.ModuleList([
                ResBlock(128, 128, timestep_emb_dim=time_embed_dim),
                TransformerBlock(128, num_heads=num_heads, patch_size=patch_size, img_size=img_size // 2),
            ]),
            ...
            nn.ModuleList([
                ResBlock(256, 256, timestep_emb_dim=time_embed_dim),
                TransformerBlock(256, num_heads=num_heads, patch_size=patch_size, img_size=img_size // 4),
            ]),
            ...
            nn.ModuleList([
                ResBlock(512, 512, timestep_emb_dim=time_embed_dim),
                ResBlock(512, 512, timestep_emb_dim=time_embed_dim),
            ]),
        ])
        ...
        self.bottleneck = nn.ModuleList([
            ResBlock(512, 512, timestep_emb_dim=time_embed_dim),
            TransformerBlock(512, num_heads=num_heads, patch_size=patch_size, img_size=img_size // 8),
            ResBlock(512, 512, timestep_emb_dim=time_embed_dim),
        ])
        ...
        self.decoder_layers = nn.ModuleList([
            ...
            nn.ModuleList([
                ResBlock(512 + 512, 512, timestep_emb_dim=time_embed_dim),
                ResBlock(512, 512, timestep_emb_dim=time_embed_dim),
            ]),
            
            nn.ModuleList([
                # ResBlock(512 + 512, 256, timestep_emb_dim=time_embed_dim),
            ...
            nn.ModuleList([
                ResBlock(256 + 256, 256, timestep_emb_dim=time_embed_dim),
                TransformerBlock(256, num_heads=num_heads, patch_size=patch_size, img_size=img_size // 4),
            ]),

            nn.ModuleList([
                # ResBlock(256 + 256, 128, timestep_emb_dim=time_embed_dim),
                # nn.Upsample(scale_factor=2),
                ResBlock(256 + 256, 256, timestep_emb_dim=time_embed_dim),
            ...
            nn.ModuleList([
                ResBlock(128 + 128, 128, timestep_emb_dim=time_embed_dim),
                TransformerBlock(128, num_heads=num_heads, patch_size=patch_size, img_size=img_size // 2),
            ]),
            
            nn.ModuleList([
                # ResBlock(128 + 128, 64, timestep_emb_dim=time_embed_dim),
                # nn.Upsample(scale_factor=2),
                ResBlock(128 + 128, 128, timestep_emb_dim=time_embed_dim),
            ...
            nn.ModuleList([
                ResBlock(64 + 64, 64, timestep_emb_dim=time_embed_dim),
                ResBlock(64, 64, timestep_emb_dim=time_embed_dim),
            ]),

            nn.ModuleList([
                ResBlock(64 + 64, first_channel, timestep_emb_dim=time_embed_dim),
            ]),
        ])
    ...
    def forward(self,
        ...
        time = self._get_timestep_embeddings(time, 64)
