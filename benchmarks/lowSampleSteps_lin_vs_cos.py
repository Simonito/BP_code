...
from generative.networks.schedulers import DDIMScheduler, NoiseSchedules
...
def perform_benchmark(model_lin, model_cos, inference_steps, val_dataset, img_out_dir, device, wandb):
    ...
    for idx, data in enumerate(tqdm(val_dataset)):
        ...
        for k in range(n):
            ...
            scheduler_lin.set_timesteps(num_inference_steps=inference_steps)
            scheduler_cos.set_timesteps(num_inference_steps=inference_steps)
...
def main(val_dataset,
    ...
    perform_benchmark(model_lin=model_lin,
                      model_cos=model_cos,
                      inference_steps=inference_steps or 1000,
